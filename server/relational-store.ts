...
import {locationsOf,defaultLocation,at,dateOf,defaultRules,packagePrice,rules,trainerHours,type Rules} from '../src/domain';
...
export function decodeRelational(snapshot:RelationalSnapshot):Database{
 ...
 for(const s of get('aco_sessions')){const earning=get('aco_earnings').find(e=>e.session_id===s.id);db.sessions.push({locationId:s.location_id||defaultLocation.id,id:s.id,clientId:s.client_id,trainerId:s.trainer_id,packageId:s.package_id||undefined,substituteId:s.substitution_id||undefined,kind:s.kind,status:s.status,date:wall(s.starts_at).slice(0,10),hour:Number(wall(s.starts_at).slice(11,13)),original:s.original_starts_at?wall(s.original_starts_at):undefined,consultationPrice:s.consultation_grosz==null?undefined:s.consultation_grosz/100,rate:earning?earning.rate_grosz/100:undefined,earned:earning?earning.amount_grosz/100:undefined,publicNote:get('aco_public_notes').find(n=>n.session_id===s.id)?.body||'',privateNote:get('aco_trainer_notes').find(n=>n.session_id===s.id)?.body||'',comments:get('aco_comments').filter(c=>c.session_id===s.id).map(c=>({id:c.id,author:c.author_label||profile(c.author_id).name||'',text:c.body,at:iso(c.created_at)!}))})}
